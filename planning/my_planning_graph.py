...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for a_node in a_level:
            for s_node in a_node.effnodes:
                is_unique = True
                for existing_s_node in s_level:
                    if s_node == existing_s_node:
                        a_node.children.add(existing_s_node)
                        existing_s_node.parents.add(a_node)
                        is_unique = False
                        break  # node is not unique, no need to continue checking
                if not is_unique:
                    continue
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff_add_a1 in node_a1.action.effect_add:
            if eff_add_a1 in node_a2.action.effect_rem:
                return True

        for eff_rem_a1 in node_a1.action.effect_rem:
            if eff_rem_a1 in node_a2.action.effect_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff_add_a1 in node_a1.action.effect_add:
            if eff_add_a1 in node_a2.action.precond_neg:
                return True

        for eff_rem_a1 in node_a1.action.effect_rem:
            if eff_rem_a1 in node_a2.action.precond_pos:
                return True

        for eff_add_a2 in node_a2.action.effect_add:
            if eff_add_a2 in node_a1.action.precond_neg:
                return True

        for eff_rem_a2 in node_a2.action.effect_rem:
            if eff_rem_a2 in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precond_a1 in preconds_a1:
            for precond_a2 in preconds_a2:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for node_a1 in actions_s1:
            for node_a2 in actions_s2:
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            s_node_goal = PgNode_s(goal, True)
            level = 0
            goal_found = False
            for s_level in self.s_levels:
                if s_node_goal in s_level:
                    level_sum += level
                    goal_found = True
                    break
                level += 1
